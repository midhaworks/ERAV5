"""Shared exact K-code head with only Qwen's final transformer block unfrozen."""
...
def run(steps=500):
    ...
    k=ExactKCodeEmbedding(tok,model.config.hidden_size).to(model.dtype); state=torch.load(root/'artifacts/qwen_exact_kcode_regression/projection.pt',map_location='cpu',weights_only=True); k.projection.data.copy_(state['projection'].to(k.projection.dtype)); model.set_input_embeddings(k); model.to(device)
    ...
    def loss_fn(ids,attention):
        hidden=model.model(input_ids=ids,attention_mask=attention).last_hidden_state[:,:-1]; states=k.state_table[ids[:,1:]]; mask=attention[:,1:].bool(); logits=torch.matmul(hidden,k.projection.t()).view(*hidden.shape[:2],k.positions,k.states); per=F.cross_entropy(logits.reshape(-1,k.states),states.reshape(-1),reduction='none').view_as(states); return per.masked_select((states!=0)&mask[...,None]).mean()
    trainable=[p for p in model.parameters() if p.requires_grad]; opt=torch.optim.AdamW(trainable,lr=1e-5); losses=[]; start=time.perf_counter(); model.train()
    ...
    result={'model':MODEL,'device':str(device),'steps':steps,'trainable_parameters':sum(p.numel() for p in trainable),'losses':losses,'loss_delta':losses[-1]-losses[0],'validation_losses':vals,'validation_mean':sum(vals)/len(vals),'finite':all(torch.isfinite(torch.tensor(losses+vals)).tolist()),'seconds':time.perf_counter()-start,'dataset_hash':manifest['dataset_hash'],'status':'exact_kcode_unfreeze_last_gate','shared_projection':True,'unfrozen_component':'final_transformer_block'}
    out=root/'artifacts/qwen_exact_kcode_unfreeze_last'; out.mkdir(parents=True,exist_ok=True); (out/'results.json').write_text(json.dumps(result,indent=2,sort_keys=True)+'\n'); print(json.dumps(result,indent=2))
